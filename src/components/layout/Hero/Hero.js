...
import DaysToSummer from '../../features/DaysToSummerAd/DaysToSummer';
...
const Hero = ({variant = '', titleText, imageSrc, ...otherProps}) => (
  <div {...otherProps} className={styles.component + variant.split(' ').map(name => ' ' + (styles[name] || name)).join('')}>
    ...
    <div className={styles.happyHour}>
      <HappyHourAd title={'HAPPY HOUR!'} promoDescription={'It is your time! Take advantage of Happy Hour! All offers 20% off!'}></HappyHourAd>
    </div>
  </div>
);
